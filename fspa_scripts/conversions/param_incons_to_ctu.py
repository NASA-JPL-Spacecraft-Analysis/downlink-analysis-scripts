...
def build_param_json_states(collection_name, data) -> None:
    ...
    for parameter in data['parameter_file']['parameter_list']:
        ...
        try:
            state = {
                ...
                "type": TYPE,
                "scet": None, # TBD but looks like we need this field
...
def build_seqgen_fincon_states(collection_name, data) -> None:
...
def main():
    ...
    parser.add_argument("-t", "--type", required=True, choices=['seqgen_fincon', 'param_json'], help="Type of input (seqgen_fincon or param_json) to publish.")
    ...
    if args.type == 'param_json':
        states = build_param_json_states(args.collection, data)
    elif args.type == 'seqgen_fincon':
        states = build_seqgen_fincon_states(args.collection, data)
    else:
