...
def main() -> None:
    ...
    valueType = ''
    if mode == 'channel':
        state_output, group_output = publish_groups.parse_channel(data)
        valueType = 'channel'
        groups = publish_groups.map_channel_groups(state_output, group_output)
        publish_groups.generate_groups(groups, state_output, collection_id, valueType, environment)
    if mode == 'parameter':
        state_output, group_output = publish_groups.parse_param(data)
